...
internal/task     Task creation and agent request parsing
...
- `internal/cli` exposes the public commands such as `tui`, `run`, `shell`, `agent`, `task`, `update`, and `version`.
- `internal/session` manages session state, process execution, PTY shells, and persisted session metadata.
- `internal/task` creates grouped agent runs and maps tasks to session IDs.
...
- Prefer small, focused changes that preserve existing session and task workflows.
